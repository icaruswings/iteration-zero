...
function ProgressBar({ status, progress }: { status: string; progress: number }) {
  return (
    ...
    <div className="space-y-4">
      {iterations
        ...
        .map((iteration) => {
          ...
          const burndown = calculateBurndownProgress(iteration.startDate, iteration.endDate ?? null, iterationTasks);
          const timeProgress = burndown.timeProgress;
          ...
          return (
            <Link
              ...
              className="block rounded-lg border border-gray-200 dark:border-gray-700 relative overflow-hidden"
            >
              <ProgressBar status={burndown.status} progress={timeProgress} />
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                      {new Date(iteration.startDate).toLocaleDateString()} - {iteration.endDate && new Date(iteration.endDate).toLocaleDateString()}
                    </div>
                  </div>
                  <div>
